...
ART = xbmc.translatePath(os.path.join('special://home/addons/script.wankstains.artwork', 'resources/pyxbmct/issues'))
...
def githubSelect(name):
    import githubissues
    githubissues.run('Colossal1/plugin.video.wankstains', '%s' % name)
    ...
    if len(items) < 1:
        msg_text = kodi.giveColor('No %s issues with Kodibox Adult at this time.' % name.title(),'deeppink',True)
    else:
        msg_text = kodi.giveColor('%s Issues with Kodibox Adult\n' % name.title(),'deeppink',True) + kodi.giveColor('Report Issues @ https://github.com/stitches5150/xxxbones/issues/','white',True) + '\n---------------------------------\n\n'
        for item in items:
