...
import os, six, re
from packlib import client, kodi, log_utils
...
search_tag = 1
search_base = urljoin(base_domain, 'search/?s=%s')


@local_utils.url_dispatcher.register('%s' % menu_mode)
def menu():
    try:
        url = urljoin(base_domain, 'vr-porn-categories/')
    ...
    for i in r:
        try:
            ...
            fanarts = xbmc.translatePath(
                os.path.join('special://home/addons/script.adultflix.artwork', 'resources/art/%s/fanart.jpg' % base_name))
            dirlst.append(
...
def content(url, searched=False):
    ...
    for i in r:
        try:
            ...
            dirlst.append({'name': name + '[COLOR yellow] [ ' + time + ' ][/COLOR]', 'url': url2, 'mode': player_mode,
                           'icon': icon, 'fanart': fanarts, 'folder': False})
        except Exception as e:
            log_utils.log('Error adding menu item. %s:: Error: %s' % (base_name.title(), str(e)), xbmc.LOGERROR)
    if dirlst:
