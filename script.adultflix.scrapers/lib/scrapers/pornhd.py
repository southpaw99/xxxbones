...
search_tag = 1
search_base = urljoin(base_domain, 'search?search=%s')
...
def menu():
    try:
        ...
        r = [(dom_parser2.parse_dom(i, 'a', req='href'), \
             ...
             for i in r if i]
        r = [
            (urljoin(base_domain, i[0][0].attrs['href']), i[1][0].attrs['alt'], i[1][0].attrs['data-original'])
            for i in r]
        if (not r):
...
def content(url, searched=False):
    try:
        ...
        r = [(dom_parser2.parse_dom(i, 'a', {'class': 'title'}), \
              ...
              i.content) \
             for i in r]
        r = [(urljoin(base_domain, i[0][0].attrs['href']), i[0][0].content, i[1][0].content, i[2]) for i in r]
    ...
    for i in r:
        try:
            ...
            try:
                ...
                icon = xbmc.translatePath(os.path.join('special://home/addons/script.adultflix.artwork',
