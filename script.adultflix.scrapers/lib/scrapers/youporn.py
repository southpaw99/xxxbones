...
def menu():
    try:
        ...
        r = re.findall('<div class="categories_list porn-categories sixteen-column action">(.*?)id="countryFlags">', c,
                       flags=re.DOTALL)[0]
    ...
    for link, icon, name in matches:
        try:
            ...
            dirlst.append(
                {'name': name, 'url': link, 'mode': content_mode, 'icon': icon, 'fanart': fanarts, 'folder': True})
        except Exception as e:
...
def content(url, searched=False):
    try:
        ...
        r = [(urljoin(base_domain, i[0][0].attrs['href']), i[1][0].content, i[2][0].content,
              i[3][0].attrs['data-original']) for i in r if i]
