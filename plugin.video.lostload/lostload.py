...
if cj:
    if os.path.isfile(xbmc.translatePath(cookiePath)):
        try:
            ...
            xbmc.log('Failed to open cookie file %s' % e,xbmc.LOGNOTICE)
...
if addon.getSetting('enable_debug') == 'true':
    enable_debug = True
    xbmc.log('lostload debug logging enabled',xbmc.LOGNOTICE)
else:
    enable_debug = False
...
class CCurrentList:
    ...
    def loadLocal(self, filename, recursive=True, lItem=None,
                  lCatcher=False):
        if enable_debug:
            xbmc.log('loadLocal: ' + str(filename),xbmc.LOGNOTICE)
            ...
            if key == 'start':
                ...
                if lCatcher:
                    try:
                        ...
                        if ret != 0:
                            if enable_debug:
                                xbmc.log('Error while loading catcher',xbmc.LOGNOTICE)
                            return ret
                    except:
                        if enable_debug:
                            traceback.print_exc(file=sys.stdout)
...
class ContentMatcher:
    def search(self, rule, contents):
        ...
        if enable_debug:
            xbmc.log('pre-action target is %s' % match,xbmc.LOGNOTICE)
        ...
        if enable_debug:
            xbmc.log('target is %s' % match,xbmc.LOGNOTICE)
...
class Main:
    def __init__(self):
        if enable_debug:
            xbmc.log('Initializing lostload',xbmc.LOGNOTICE)
        self.pDialog = None
        self.urlList = []
        self.extensionList = []
        self.selectionList = []
    ...
    def determinePreferredLink(self):
        ...
        if enable_debug:
            xbmc.log('found %d urls, prefered video type: %d'
                % (len(self.urlList), preference),xbmc.LOGNOTICE)
    ...
    def playVideo(self, videoItem):
        ...
        if self.currentlist.skill.find('nodownload') == -1:
            if addon.getSetting('download') == 'true':
                ...
                if xbmcgui.Dialog().yesno('', __language__(30052)):
                    ...
                    self.pDialog.close()
                    if flv_file is None:
                        xbmcgui.Dialog().ok('lostload Info',
                                            __language__(30053))
        else:
            flv_file = None
        ...
        if flv_file != None and os.path.isfile(flv_file):
            if enable_debug:
                xbmc.log('Play: ' + str(flv_file),xbmc.LOGNOTICE)
            xbmc.Player().play(str(flv_file), listitem)
        else:
            if enable_debug:
                xbmc.log('Play: ' + str(url),xbmc.LOGNOTICE)
    ...
    def downloadMovie(self, url, title):
        url = url.split('|')[0]
        if enable_debug:
            xbmc.log('Trying to download video ' + str(url),xbmc.LOGNOTICE)
    ...
    def parseView(self, url):
        ...
        if ext == 'cfg' or ext == 'list':
            ...
            return -2
        elif ext == 'lostload' or ext == 'dwnlddevil':
            url = urllib.unquote_plus(url)
            url = url[:len(url) - 11]
           ...
           and self.currentlist.videoCount() == 1:
            ...
            for m in self.currentlist.items:
                ...
                if m_type == 'rss' or m_type == 'search':
                    ...
                    self.addListItem(m_title,
                                     self.currentlist.codeUrl(m, 'lostload'),
                                     m_icon,
                                     len(self.currentlist.items),
                                     m)
        return result
    ...
    def addListItem(self, title, url, icon, totalItems, lItem):
        ...
        if self.currentlist.getFileExtension(url) == 'lostload' \
    ...
    def run(self):
        if enable_debug:
            xbmc.log('lostload running',xbmc.LOGNOTICE)
        try:
            ...
            if len(paramstring) <= 2:
                ...
                if enable_debug:
                    ...
                    xbmc.log('Cache directory: ' + str(cacheDir),xbmc.LOGNOTICE)
                    xbmc.log('Resource directory: ' + str(resDir),xbmc.LOGNOTICE)
                    xbmc.log('Image directory: ' + str(imgDir),xbmc.LOGNOTICE)
                if not os.path.exists(settingsDir):
                    if enable_debug:
                        xbmc.log('Creating settings directory ' + str(settingsDir),xbmc.LOGNOTICE)
                    os.mkdir(settingsDir)
                    if enable_debug:
                        xbmc.log('Settings directory created',xbmc.LOGNOTICE)
                if not os.path.exists(cacheDir):
                    if enable_debug:
                        xbmc.log('Creating cache directory ' + str(cacheDir),xbmc.LOGNOTICE)
                    os.mkdir(cacheDir)
                    if enable_debug:
                        xbmc.log('Cache directory created',xbmc.LOGNOTICE)
                if enable_debug:
            ...
            if enable_debug:
                traceback.print_exc(file=sys.stdout)
            xbmcgui.Dialog().ok('lostload Error',
                'Error running lostload.\n\nReason:\n' + str(e))
