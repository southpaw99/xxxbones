...
search_tag = 1
search_base = urljoin(base_domain, 'search_videos/?q=%s')
...
def menu():
    try:
        ...
        r = [(dom_parser2.parse_dom(i, 'a', req='href'), \
             ...
             for i in r]
        r = [(i[0][0].attrs['href'], i[1][0].content.replace('(', '[ ').replace(')', ' ]'), i[2][0].attrs['data-src'])
             for i in r if i]
        if (not r):
...
def content(url, searched=False):
    try:
        ...
        r = [(dom_parser2.parse_dom(i, 'a', req=['href', 'title']), \
              ...
              dom_parser2.parse_dom(i, 'img', req='data-src')) \
             for i in r]
        r = [(i[0][0].attrs['href'], i[0][0].attrs['title'], i[1][0].content, i[2][0].attrs['data-src']) for i in r if
             i]
