...
def backup_fav():
    ...
    backup_content = {"meta": {"type": "bwg-favorites", "version": 1, "datetime": time}, "data": favorites}
    if progress.iscanceled():
        progress.close()
        return
    progress.update(75, "Writing backup file")
    filename = "bwg-favorites_" + time + '.bak'
    try:
...
def restore_fav():
    ...
    try:
        ...
        return
    if not backup_content["meta"]["type"] == "bwg-favorites":
        utils.notify("Error", "Invalid backup file")
        return
