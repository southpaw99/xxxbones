...
import os, six, re
from packlib import log_utils, kodi, cache, client, dom_parser2
...
def menu():
    try:
        ...
        r = [(urljoin(base_domain, i.attrs['href']), i.content) for i in r if '/tag' in i.attrs['href']]
        if (not r):
    ...
    if r:
        for i in r:
            try:
                iconimage = xbmc.translatePath(os.path.join('special://home/addons/script.adultflix.artwork',
                                                            'resources/art/%s/icon.png' % filename))
                fanarts = xbmc.translatePath(os.path.join('special://home/addons/script.adultflix.artwork',
                                                          'resources/art/%s/fanart.jpg' % filename))
                name = kodi.sortX(i[1].encode('utf-8'))
                dirlst.append({'name': name, 'url': i[0], 'mode': content_mode, 'icon': iconimage, 'fanart': fanarts,
                               'folder': True})
            except Exception as e:
...
def content(url, searched=False):
    try:
        ...
        r = [(urljoin(base_domain, i[0][0].attrs['href']), i[0][0].attrs['title'], i[1][0].content, \
              i[2][0].attrs['data-original'] if i[2][0].attrs['data-original'].startswith('http') else 'https:' +
                                                                                                       i[2][0].attrs[
                                                                                                           'data-original'])
             for i in r if i[2][0].attrs['data-original']]
    ...
    for i in r:
        try:
            ...
            dirlst.append({'name': name, 'url': content_url, 'mode': player_mode, 'icon': i[3], 'fanart': fanarts,
