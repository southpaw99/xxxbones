...
def menu():
    try:
        ...
        r = [(urljoin(base_domain, i[0][0].attrs['href']), i[1][0].attrs['title'], i[1][0].attrs['src']) for i
             in r if i]
        if (not r):
...
def content(url, searched=False):
    try:
        ...
        r = [(dom_parser2.parse_dom(i, 'h1', {'class': 'mtitle'}, req='title'),
              dom_parser2.parse_dom(i, 'div', {'class': 'thumb_container'})) for i in r if i]
        r = [(i[0][0].attrs['title'], dom_parser2.parse_dom(i[0][0].content, 'a', req='href'), \
              dom_parser2.parse_dom(i[1][0].content, 'img', req='src'))
             for i in r if i[0]]
        r = [(i[0], urljoin(base_domain, i[1][0].attrs['href']), i[2][0].attrs['src']) for i in r if i]
