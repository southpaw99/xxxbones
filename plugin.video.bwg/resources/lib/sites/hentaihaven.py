...
def Main():
    utils.addDir('[COLOR white]Categories[/COLOR]', 'http://hentaihaven.org/pick-your-poison/', 463, '', '')
    utils.addDir('[COLOR white]A to Z[/COLOR]', 'http://hentaihaven.org/pick-your-series/', 464, '', '')
    utils.addDir('[COLOR white]Uncensored[/COLOR]', 'http://hentaihaven.org/ajax.php?action=pukka_infinite_scroll&page_no=1&grid_params=infinite_scroll=on&infinite_page=2&infinite_more=true&current_page=taxonomy&front_page_cats=&inner_grid%5Buse_inner_grid%5D=on&inner_grid%5Btax%5D=post_tag&inner_grid%5Bterm_id%5D=53&inner_grid%5Bdate%5D=&search_query=&tdo_tag=uncensored&sort=date', 461, '', '')
...
def A2Z(url):
    ...
    for catpage, name, img, other in match:
        if 'uncensored' in other:
            name = name + " [COLOR white]Uncensored[/COLOR]"
