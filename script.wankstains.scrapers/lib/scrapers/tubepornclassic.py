...
base_domain  = 'http://www.tubepornclassic.com'
...
def menu():
    ...
    for i in r:
        try:
            ...
            dirlst.append({'name': name, 'url': urlparse.urljoin(base_domain,'search/%s/' % i[1].lower()), 'mode': content_mode, 'icon': icon, 'fanart': fanarts, 'folder': True})
        except Exception as e:
...
def content(url,searched=False):
    ...
    try:
        ...
        r = [(dom_parser2.parse_dom(i, 'a', req=['href','title']), \
             ...
             for i in r if i]
        r = [(i[0][0].attrs['href'], i[0][0].attrs['title'], i[1][0].content, i[2][0].attrs['data-original']) for i in r if i[1]]
    ...
    if dirlst:
        if not searched:
            try:
                ...
                dirlst.append({'name': 'Next Page -->', 'url': url, 'mode': content_mode, 'icon': xbmc.translatePath(os.path.join('special://home/addons/script.wankstains.artwork', 'resources/art/main/next.png')), 'fanart': fanarts, 'description': 'Load More Videos...', 'folder': True})
            except: pass
