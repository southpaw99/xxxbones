...
search_tag = 1
search_base = urljoin(base_domain, 'video/search?query=%s')
...
def menu():
    try:
        ...
        r = [(dom_parser2.parse_dom(i, 'a', req='href'), \
             ...
             for i in r if i]
        r = [(urljoin(base_domain, i[0][0].attrs['href']), i[1][0].attrs['alt'],
...
def content(url, searched=False):
    try:
        ...
        r = [(dom_parser2.parse_dom(i, 'a', req='href'), \
             ...
             for i in r if i]
        r = [(urljoin(base_domain, i[0][0].attrs['href']), i[1][0].attrs['alt'],
              urljoin(base_domain, i[1][0].attrs['src']), i[2][0].content if i[2] else 'Unknown') for i in r]
