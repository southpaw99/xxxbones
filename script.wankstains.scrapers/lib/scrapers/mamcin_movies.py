...
def content(url):
    ...
    try:
        if not url: url = 'https://www.mamcin.com/category/non-classe/'
    ...
    for i in r:
        try:
            ...
            title = title.replace('Plus Belle La Vie Épisode', 'Episode')
            name = title
            # name = name.replace('Plus Belle La Vie Épisode','Épisode')
            log_utils.log(title, log_utils.LOGERROR)
            description = name
