...
def buildDir(items, content='dirs', cm=None, search=False, stopend=False, isVideo = False, isDownloadable = False, cache=True, chaturbate=False, pictures=False):
    ...
    for i in items:
        ...
        try:
            ...
            open_set = '%s?mode=%s' \
            % (sysaddon,str('19'))
            stext = "Open Kodibox Adult Settings"
...
def depVersions():
    ...
    try:
        xml1 = client.request('https://raw.githubusercontent.com/stitches5150/xxxbones/master/zips/addons.xml')
        xml2 = client.request('https://raw.githubusercontent.com/stitches5150/xxxbones/master/zips/addons.xml')
        lst = [('plugin.video.wankstains'),('script.wankstains.scrapers'),('script.wankstains.artwork'), \
...
def viewDialog(url):
    ...
    window = TextBox('Kodibox Adult')
    window.doModal()
    del window

class TextBox(pyxbmct.AddonDialogWindow):

    def __init__(self, title='Kodibox Adult'):
    ...
    def set_info_controls(self):
        ...
        Background   = pyxbmct.Image(xbmc.translatePath(os.path.join('special://home/addons/script.wankstains.artwork', 'resources/art/dialog/bg.jpg')))
...
def hard_reset():
    ...
    choice = xbmcgui.Dialog().yesno("[COLOR orangered][B]RESET Kodibox Adult?[/B][/COLOR]", '[COLOR white]ARE YOU SURE YOU WANT TO RETURN Kodibox Adult TO THE DEFAULT STATE AND LOSE ALL YOUR INFORMATION?[/COLOR]')
    if choice:
        try:
            ...
            quit()
        kodi.dialog.ok(kodi.get_name(), "[COLOR white]Kodibox Adult has been reset to the factory state.[/COLOR]","[COLOR white]Press OK to continue.[/COLOR]")
