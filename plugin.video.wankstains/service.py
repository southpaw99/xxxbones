...
try:
    ...
    databases = xbmc.translatePath(os.path.join('special://profile/addon_data/plugin.video.wankstains', 'databases'))
    chaturbatedb = xbmc.translatePath(os.path.join(databases, 'chaturbate.db'))

    xbmc.log('Starting Kodibox Adult Service', xbmc.LOGNOTICE)
    ...
    if ( chat_on_off ) != 'true': 
        xbmc.log('Exiting Kodibox Adult Service', xbmc.LOGNOTICE)
        quit()
    ...
    xbmc.log('Starting Kodibox Adult Chaturbate Monitoring', xbmc.LOGNOTICE)

    while not xbmc.abortRequested:

        if not i:
            #xbmc.log('Getting Chaturbate Monitored Performers from Kodibox Adult', xbmc.LOGNOTICE)
            conn = sqlite3.connect(chaturbatedb)
            ...
            for (title, link, iconimage) in e:
                if (not title in str(lst) ):
                    #xbmc.log('Kodibox Adult: Checking %s' % title, xbmc.LOGNOTICE)
                    try:
        ...
        if i >= 2879:
            lst = []
            xbmc.log('Kodibox Adult: Reseting Checked List', xbmc.LOGNOTICE)
        i = (i + 1) % 60
        j = (j + 1) % 2880
except: pass