...
class indexer:
    ...
    def root(self):
        try:
            regex.clear()
            url = 'https://addoncloud.org/drakon/XMLS/Home.xml'
            self.list = self.TAD_list(url)
    ...
    def getx(self, url, worker=False):
        try:
            ...
            url = regex.resolve(r)
            self.list = self.TAD_list('', result=url)
            self.addDirectory(self.list)
            return self.list
        except:
            pass
    ...
    def developer(self):
        try:
            ...
            self.list = self.TAD_list('', result=result)
    ...
    def addSearch(self, url=None):
        try:
            ...
            self.list = [] ; threads = [] 
            for link in links: threads.append(workers.Thread(self.TAD_list, link))
            ...
            for i in self.list:
                try:
                    name = ''
                    if not i['vip'] in ['TAD TV']: name += '[B]%s[/B] | ' % i['vip'].upper()
                    name += i['name']
                    i.update({'name' : name})
                except:
                    pass
    ...
    def TAD_list(self, url, result=None):
        try:
