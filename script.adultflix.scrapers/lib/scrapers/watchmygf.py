...
def menu():
    try:
        ...
        r = [(i[0][0].attrs['href'], i[1][0].attrs['alt'], i[1][0].attrs['src'],
              re.findall('.+?(\d+)', re.sub('<.+?>', '', i[2][0].content))[0]) for i in r if i]
        if (not r):
...
def pic_menu(url=None):
    try:
        if (not url): url = urljoin(base_domain, 'photos/popular/')
        c = client.request(url)
    ...
    for i in r:
        try:
            ...
            fanarts = xbmc.translatePath(
                os.path.join('special://home/addons/script.adultflix.artwork', 'resources/art/%s/fanart.jpg' % filename))
            dirlst.append(
                {'name': name, 'url': i[0], 'mode': pic_con_mode, 'icon': i[2], 'fanart': fanarts, 'folder': True})
        except Exception as e:
            log_utils.log('Error adding menu item %s in %s:: Error: %s' % (i[1].title(), base_name.title(), str(e)),
                          xbmc.LOGERROR)
...
@local_utils.url_dispatcher.register('%s' % pic_con_mode, ['url'])
def pic_content(url):
    try:
        c = client.request(url)
    ...
    for i in r:
        try:
            ...
            fanarts = xbmc.translatePath(
                os.path.join('special://home/addons/script.adultflix.artwork', 'resources/art/%s/fanart.jpg' % filename))
            dirlst.append(
                {'name': name, 'url': i[0], 'mode': pic_v_mode, 'icon': i[0], 'fanart': fanarts, 'folder': True})
        except Exception as e:
            log_utils.log('Error adding menu item %s in %s:: Error: %s' % (i[1].title(), base_name.title(), str(e)),
                          xbmc.LOGERROR)
