...
def content(url,searched=False):
    ...
    try:
        c = client.request(url)
        r = dom_parser2.parse_dom(c, 'li', {'class': re.compile('videoblock\svideoBox')})
        r = [(dom_parser2.parse_dom(i, 'a', req=['href','title']), \
            ...
            for i in r if i]
        r = [(urlparse.urljoin(base_domain,i[0][0].attrs['href']), i[0][0].attrs['title'], i[1][0].content, i[2][0].attrs['data-mediumthumb']) for i in r if i[2]]
