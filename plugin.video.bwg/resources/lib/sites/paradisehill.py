...
def Main():
    utils.addDir('[COLOR white]Categories[/COLOR]','http://en.paradisehill.cc/porn/',253,'','')
    utils.addDir('[COLOR white]Search[/COLOR]','https://en.paradisehill.cc/search/?pattern=',254,'','')
...
def Cat(url):
	...
	match1 = re.compile('href="([^"]+)".+?<span.+?<span>(.+?)<.+?img src="([^"]+)"', re.DOTALL | re.IGNORECASE).findall(match[0])
