...
ART = xbmc.translatePath(os.path.join('special://home/addons/script.wankbank.artwork', 'resources/pyxbmct/issues'))
...
def githubSelect(name):
    import githubissues
    githubissues.run('Colossal1/plugin.video.wankbank', '%s' % name)
    ...
    if len(items) < 1:
        msg_text = kodi.giveColor('No %s issues with Wank Bank at this time.' % name.title(),'deeppink',True)
    else:
        msg_text = kodi.giveColor('%s Issues with Wank Bank\n' % name.title(),'deeppink',True) + kodi.giveColor('Report Issues @ https://github.com/Colossal1/plugin.video.wankbank/issues','white',True) + '\n---------------------------------\n\n'
        for item in items:
